...
void EditorMapDef::InsertStepBefore( MapGenStep* stepToInsert, int indexToInsertBefore ) {
    if( indexToInsertBefore < 0 ) {
        return;
    } else if( indexToInsertBefore > m_numSteps - (NUM_POST_STEPS + 1) ) {
        m_mapGenSteps.push_back( stepToInsert );
    } else {
        std::vector< MapGenStep* >::iterator stepIter = m_mapGenSteps.begin() + indexToInsertBefore - NUM_PRE_STEPS;
...
void EditorMapDef::InsertStepAfter( MapGenStep* stepToInsert, int indexToInsertAfter ) {
    InsertStepBefore( stepToInsert, indexToInsertAfter + 1 );
}
...
int EditorMapDef::SetupChangeTileMGS() {
    ...
    return MapGenStep::AddCustomResult( EVENT_EDITOR_CHANGE_TILE, attributes, REQUIRE_NONE );
}
