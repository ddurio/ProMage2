...
typedef std::map< std::string, Strings > AttributeMap;
...
class MapGenStep {
    ...
    explicit MapGenStep( const Strings& motifHierarchy );
    virtual ~MapGenStep() {};

    static int AddCustomCondition( const std::string& eventName, const AttributeMap& attributes, CustomAttrRequirement requirement = REQUIRE_ALL );
    static int AddCustomResult( const std::string& eventName, const AttributeMap& attributes, CustomAttrRequirement requirement = REQUIRE_ALL );
    ...
    static MapGenStep* CreateMapGenStep( const std::string& stepType, const Strings& motifHierarchy );
    ...
    virtual void UpdateParentMotifs( const Strings& parentMotifHierarchy );
    ...
    struct CustomEvent {
        ...
        Strings attrNames;
        Strings attrValues;
        std::vector< Strings > allowedValues;
