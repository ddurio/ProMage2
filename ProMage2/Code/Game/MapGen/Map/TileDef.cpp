...
Strings TileDef::GetAllTypesIgnoreEdgedVariants() {
    Strings allDefTypes;
    ...
    for( defIter; defIter != s_definitions.end(); defIter++ ) {
        if( !StringICmp( defIter->second->m_tileContext, "edgedVariant" ) ) {
...
void TileDef::GetUVsInverted( Vec2& uvTL, Vec2& uvBR ) const {
    uvTL = Vec2( m_uvCoords.mins.x, m_uvCoords.maxs.y );
    uvBR = Vec2( m_uvCoords.maxs.x, m_uvCoords.mins.y );
}
