...
Actor::Actor( Map* theMap, const std::string& actorType, const std::string& controllerType ) :
    ...
    // Player Controller - (new controllers should also be added to SetupSpawnActorMGS)
...
int Actor::SetupSpawnActorMGS() {
    std::map< std::string, Strings > attributes;
    attributes["spawnActor"] = ActorDef::GetAllDefinitionTypes();
    attributes["controller"] = { "Player", "Merchant", "Enemy", "Credits" };

    return MapGenStep::AddCustomResult( EVENT_SPAWN_ACTOR, attributes, REQUIRE_ONE );
}
...
void Actor::Update( float deltaSeconds ) {
    ...
    if( IsAlive() ) {
        ...
        // Show health bar
        UpdateHealthBar();
    } else if( m_deathTimer != nullptr && m_deathTimer->HasFinished() ) {
