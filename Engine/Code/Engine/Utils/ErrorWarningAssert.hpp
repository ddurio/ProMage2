...
//	#define ERROR_AND_DIE( errorText )						// "MUST not reach this point"; Show error dialog, then shut down the app
//	#define ERROR_RECOVERABLE( errorText )					// "SHOULD not reach this point"; Show warning dialog, then proceed
//	#define GUARANTEE_OR_DIE( condition, errorText )		// "MUST be true"; If condition is false, show error dialog then shut down the app
//	#define GUARANTEE_RECOVERABLE( condition, errorText )	// "SHOULD be true"; If condition is false, show warning dialog then proceed
...
//	- Opens an error/message dialog box
...
//	- Opens an warning/message dialog box
//	- Triggers a debug breakpoint (if appropriate development suite is present)
//	- Continues execution
//
...
//
#if defined( DISABLE_ASSERTS )
#define ASSERT_RETURN_VALUE( condition, errorMessageText ) { (void)( condition ); }
#else
#define ASSERT_RETURN_VALUE( condition, returnValue )	\
{														\
	if( !(condition) )									\
	{													\
		return returnValue;                 			\
