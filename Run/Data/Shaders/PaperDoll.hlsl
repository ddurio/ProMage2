...
Texture2D<float4> tHat      : register(t0);
...
float4 FragmentFunction( VertexToFragment input ) : SV_Target0 {
    ...
    float4 hatColor           = tHat.Sample( sColor, input.uv );
    ...
    float hatFactor           = saturate( feetFactor     - feetColor.w );
    float hairFactor          = saturate( hatFactor      - hatColor.w );
    ...
    float4 finalHatColor      = hatColor      * hatFactor;
    ...
    float4 finalTexColor = finalBodyColor + finalEarsColor + finalChestColor + finalShoulderColor + finalHairColor + finalHatColor + finalLegsColor + finalFeetColor + finalWeaponColor;
