...
void EditorMapGenStep::RenderConditions_CellularAutomata( MapGenStep* genStep ) {
    ...
    std::array< bool, 2 > change3and4 = RenderTagsOrVar( uniqueKey, stepVars, "ifNeighborHasTags", genStep->m_motifHierarchy, caStep->m_ifNeighborHasTags, paramsChanged[11], "Neighbor" ); // FIXME: hard coded changed value
...
void EditorMapGenStep::RenderConditions_Custom( MapGenStep* genStep ) {
    ...
    for( int varIndex = 0; varIndex < numVars; varIndex++ ) {
        ...
        if( StringICmp( varType, "String" ) ) {
            ...
            wasChanged = ImGui::InputText( varName.c_str(), &currentValue );
